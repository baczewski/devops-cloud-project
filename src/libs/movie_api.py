...
class MovieAPI:
    ...
    async def get_trending_movies(self, time_window: TimeWindow = TimeWindow.DAY) -> list[Media]:
        results = await self.__get_trending_media("movie", time_window)
        return [Media(res["title"], res["vote_average"]) for res in results]

    async def get_trending_shows(self, time_window: TimeWindow = TimeWindow.DAY) -> list[Media]:
        results = await self.__get_trending_media("tv", time_window)
        return [Media(res["name"], res["vote_average"]) for res in results]

    async def __get_trending_media(self, media_type: str, time_window:TimeWindow) -> list:
        time_window_str = self.__get_time_window_str(time_window)

        url = f"https://api.themoviedb.org/3/trending/{media_type}/{time_window_str}"
    ...
    async def __request_get_async(self, url: str) -> requests.Response:
        ...
        return await loop.run_in_executor(None, lambda: requests.get(url, headers=self.__headers, timeout=5))
